...
% r: Read counts normalized by read count from paired normal sample
% snv_baf: BAFs of SNPs
% snv_seg: Membership of SNP to segment
% m: No of intervals
% n: Number of subclones 
% K: Max number of CNAs
% neta: Average coverage
% baf_t: Score threshold on how fraction of SNVs that should be consistent
% baf_bnd: Range of error within which BAF is considered to be consistent
% num_iter: Max number of iterations 


function [mu_est,L_est] = alt_min_fn_snp(r,snv_baf,snv_seg,m,n,K,neta,baf_t,baf_bnd,num_iter)
...
h = waitbar(iter/num_iter,'AMTHet Running');
...
%     L_prev = L_curr;
    ...
    for i = 1:m         % Same process for each region
        snv_i = snv_baf(snv_seg==i);  % Pulling out the BAFs for current segment  
        d_i = d(i);   % Error bound
        ...
        while true
            ...
            elseif ~incr
                ...
                if level == n-1   % Reached a leaf node
                    ...
                    if d_red < d_i  % If solution is better
                        baf_score = score_baf(snv_i,L_tmp,mu_tum,baf_bnd);  % Check for consistency on BAFs
                        if baf_score>baf_t
                            d_i = d_red;    % Improved bound for search
                            L_i = L_tmp;
                            %fprintf('SOLUTION ACCEPTED with BAF score %.3f \n',baf_score);
                         else 
                            %fprintf('SOLUTION DISCARDED \n');
                        end
                    end
                    incr = true;
